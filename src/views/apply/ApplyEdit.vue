<template>
  ...
  <el-dialog v-model="innerVisible" title="" width="430px" close-icon="false">
    <el-space
      size="24"
      direction="vertical"
      alignment="center"
    ...
    >
      ...
      >
        <div class="status-desc">应用创建成功</div>
        <div class="waring-text">可返回列表或继续上架当前应用</div>
      </el-space>
      <el-space
        size="8"
        ...
        style="width: 100%; margin-bottom: 8px"
      >
        <el-button type="primary">上架应用</el-button>
...
const changeFile = async (fileType, uploadFile) => {
  ...
  if (namespaceId && uploadFile) {
    uploadFile.raw.chunks = ["1"];
    console.log(uploadFile, "--uploader-");
    try {
      const uploader = await $application.uploads(uploadFile.raw, {
        namespaceId,
      });
